...
if authentication_status:
    ...
    if page == "Main screen":
        ...
        container.text_area(label="Plain Language Summary", value=st.session_state.draft_response_content, height=350)
        ...
        education_level = st.radio(
            ...
            ['Elementary', 'High School', 'Bachelors', 'Masters', 'Above PhD'],
            index=['Elementary', 'High School', 'Bachelors', 'Masters', 'Above PhD'].index(
                user_data["education_level"]) if user_data and "education_level" in user_data else 0
        )
        ...
        if st.button("Update"):
            db.put(
                {"key": username, "api": key.encrypt(bytes(api_input, 'utf-8')).decode(),
                 "writing_style": writing_style, "education_level": education_level,
                 "domain": domain})
